...
interface Props {
    supplier: Supplier;
    supplierVarieties: VarietyItem[];
    speciesList: Species[];
    varietiesList: Variety[];
}

// Función para convertir VarietyItem[] a SpeciesGroup[]
function convertToSpeciesGroups(items: VarietyItem[]): SpeciesGroup[] {
    const groupMap = new Map<string, string[]>();

    items.forEach(item => {
        const existing = groupMap.get(item.species_name) || [];
        if (!existing.includes(item.variety_name)) {
            existing.push(item.variety_name);
        }
        groupMap.set(item.species_name, existing);
    });

    return Array.from(groupMap.entries()).map(([species_name, varieties]) => ({
        species_name,
        varieties,
    }));
}

export default function EditSupplier({ supplier, supplierVarieties, speciesList, varietiesList }: Props) {
    // Estado para grupos de especies con sus variedades
    const [speciesGroups, setSpeciesGroups] = useState<SpeciesGroup[]>(
        convertToSpeciesGroups(supplierVarieties || [])
    );
